...
success: function (response) {
   // console.log(response);
    ...
    if(response[i].hasOwnProperty("m_name")){
        ...
        $('.optional>.opcont'+i).append("<div class='details'><h3 id='name'>"+response[i]['name']+"</h3><span>"+response[i]['flipkardprice']['amount']+" "+response[i]['flipkardprice']['currency']+"</span> <span> " +response[i]['discount']+ "%off</span><br><span class='offer'>Flipkard "+response[j]['m_specialprice']['amount']+"/- "+response[j]['m_flipkardprice']['currency']+"<a target='_blank' href="+response[i]['producturl']+" class='buynow'>BUY NOW</a></div>")
    }
    }
    if($('.imgrate .'+b).html()==''){
